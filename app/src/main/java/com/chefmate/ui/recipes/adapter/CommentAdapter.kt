...
class CommentAdapter(
    ...
    private val depth: Int = 0 // Track nesting depth for compact display
    ...
    fun refreshRepliesForComment(commentId: Long, newReplies: List<Comment>) {
        // This method will be called from the parent to update replies
        // We need to find the comment and update its replies
    ...
    class CommentViewHolder(
        ...
        fun bind(comment: Comment) {
            // Make nested replies more compact
            if (depth > 0) {
                // Reduce card margin for nested replies
                ...
                // Make profile image smaller for nested replies
                ...
                // Reduce text size slightly for nested replies
                if (depth > 1) {
            ...
            // Display username in lowercase, handle null/empty
            val username = comment.userName.ifEmpty { "Неизвестен потребител" }
            authorTextView.text = username.lowercase()
            contentTextView.text = comment.content.ifEmpty { "(празен коментар)" }
            ...
            // Make author and profile image clickable - only if userId is valid
            if (comment.userId > 0) {
            ...
            // Reply button - show for all comments (can reply to any comment)
            ...
            // Replies - show with visual indicator (arrow/divider)
            ...
            if (replies.isNotEmpty()) {
                ...
                // Reuse existing adapter if possible, otherwise create new one
                ...
                if (existingAdapter != null) {
                    ...
                    // Create new adapter with increased depth for nested replies
                    ...
                    val repliesAdapter = CommentAdapter(
                        ...
                        onDeleteClick,
                        currentUserId,
                        depth + 1 // Increment depth for nested replies
                    )
